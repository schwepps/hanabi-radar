-- Partners may update `items.status` (mark processed / dismissed).
--
-- The partner RLS layer deliberately kept `items` read-only for partners (SELECT policy only —
...
--   * an UPDATE policy gated by the existing is_partner() predicate,
...
--     writes `status` — would be born hidden for everyone with no UI to
--     recover them.
...
comment on policy items_update_status_partner on items is
  'Active partners may UPDATE status on displayed rows only (column grant limits columns to status; row scope limits to classified streams). Shared workflow — a dismiss/process is collective, not per-partner.';
